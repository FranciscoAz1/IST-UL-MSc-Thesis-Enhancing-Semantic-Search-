...
To validate the schema, we inserted a small synthetic corpus with enumerated placeholders and explicit cross-references, aligned with the UML in Figure~\ref{fig:weaviate_class}. Concretely: \textit{Entidade} (Empresa 1--2); for each \textit{Entidade}, 5 \textit{Pasta} (e.g., \enquote{Pasta 3 of Empresa 2}); for each \textit{Pasta}, 8 \textit{Ficheiro} (e.g., \enquote{Documento 4 in Pasta 3 of Empresa 2}); 5 \textit{Fluxo} (Fluxo 1--5); and for each \textit{Fluxo}, 10 \textit{Etapa} (Etapa 1--10 of Fluxo i). In this implementation, \textit{Metadados} is modeled as a property on \textit{Ficheiro} (no separate \textit{Metadados} objects are instantiated). Objects are linked as follows: \textit{Entidade}~\textrightarrow~\texttt{hasPastas}/\texttt{hasFicheiros}; \textit{Pasta}~\textrightarrow~\texttt{hasEntidades}/\texttt{hasFicheiros}; \textit{Fluxo}~\textrightarrow~\texttt{belongsToPastas}/\texttt{belongsToFicheiros} and \textit{Etapa}~\(\leftrightarrow\)~\textit{Fluxo} via \texttt{belongsToFluxo}/\texttt{hasEtapas}; \textit{Etapa}~\textrightarrow~\texttt{hasFicheiros}. Names are placeholders (\enquote{Empresa 1}, \enquote{Pasta 2 of Empresa 1}, etc.), ensuring the evaluation targets traversal and latency rather than content semantics. The following test queries illustrate structure-aware and semantic retrieval:
\begin{itemize}
