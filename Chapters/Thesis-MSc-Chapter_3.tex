...
This chapter surveys \glsxtrfull{AAI}, language-model driven agents that plan, use tools, and manage memory, to improve retrieval and answer quality. We focus on methods that (i) reason and decompose tasks, (ii) call external tools and data sources, and (iii) maintain working åand long-term memory. We also connect these capabilities to \glsxtrfull{RAG} pipelines that must respect organizational structure (folders, metadata) and access control.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|c|cc}
        ...
        \hline
    \end{tabular}
    \caption{Experiment curated results using PALM (540B) \glsxtrfull{LLM} model \cite{chainofzero}}
...
For enterprise semantic search, \glsxtrfull{AAI} adds:
