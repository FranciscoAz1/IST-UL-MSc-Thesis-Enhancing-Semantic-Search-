...
\begin{itemize}
    \item weaviate query : fetch objects throught a hybrid query (vector and BM25 search). The vector searches are ranked 70 \% higher than BM25. To use mostly semantic, but also BM25 when semantic searches are not good.
    \item Weaviate generation : Same as weaviate query, but uses weaviate database to generate, and returns generation instead of objects
    \item Weaviate query cross-references : same as weaviate query, but finds cross-references, and suggest searching throught them
    \item  weaviate query with cross-reference : Follows the last query. Searches a collectiong and the selected cross-reference for information. Then suggests other cross references to search through. Guiding the model to search the database in a structured way.
\end{itemize}
